...
async def run():
    ...
    async def fly_to_position_velocity(target_n, target_e, target_d, ignore_height=True, n_threshold=N_THRESHOLD, e_threshold=E_THRESHOLD, d_threshold=D_THRESHOLD,scale=KP_SCALE,test=False):
        ...
        while True:
            ...
            if state == False:
                ...
                if test == False:
                    await send_velocity(0.0, 0.0, 0.0)  # Stop movement if UWB data is not ready
                await asyncio.sleep(0.5)
                continue
    ...
    try:
        ...
        # --- SITL-only: real hardware takes off manually via RC before this
        # script runs; in SITL we must auto-takeoff to TAKEOFF_HEIGHT first.
        print(f"SITL auto-takeoff to {TAKEOFF_HEIGHT} m...")
        await drone.action.takeoff()
        async for pos in drone.telemetry.position_velocity_ned():
            if pos.position.down_m < -(TAKEOFF_HEIGHT - 0.1):
                print(f"  reached down_m={pos.position.down_m:.2f}")
                break
        await asyncio.sleep(1.0)
        # --- end SITL-only block
